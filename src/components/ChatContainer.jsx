    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
...

        {
          messagesDummyData?.map((message, index) => (
            <div key={index} className={`flex items-end gap-2 justify-end ${message?.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'} `}>
              {message?.image ? (
                <img src={message?.image} alt="message-image" className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8' />
              ) : (
              ...
              <div className='text-center text-xs'>
                <img src={message?.senderId === '680f50e4f10f3cd28382ecf9' ? assets?.avatar_icon : assets?.profile_martin} alt="user" className='w-7 rounded-full' />
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          ...
          <input type="file" id='image' accept='image/png, image/jpg' hidden className='' />
          <label htmlFor="image">
            <img src={assets?.gallery_icon} alt="gallery-icon" className='w-5 mr-2 cursor-pointer ' />
          </label>
        </div>

        <img src={assets?.send_button} alt="send message" className='w-7 cursor-pointer' />
